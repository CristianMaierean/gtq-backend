...
- **Thumbnails are generated in-house, not pulled from source articles.**
  `src/thumbnail.js` renders a branded SVG per article (category icon,
  headline, GamerTech wordmark) entirely in code — no third-party photos
  involved, so there's no copyright exposure from reusing an outlet's (or
  even a manufacturer's) image. Uploaded to Shopify as a file attachment,
  same as any other image. An earlier version of this pulled the source
  article's `og:image`; that approach was deliberately dropped over the
  copyright question — see git history on `src/fetch-article.js` if curious.
...
| `src/thumbnail.js` | Generates the branded SVG thumbnail per article |
